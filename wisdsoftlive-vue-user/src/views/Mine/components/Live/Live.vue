<template>
	<div class="body">
		...
		<div v-show="haveData" class="body_02">
			<div class="body_03" v-for="item in purchasedLiveCourses" :key="item.id">
				<a-card hoverable class="myCard">
					<img class="courseImg" alt="example" :src="item.url" />
					...
					<template slot="actions" class="ant-card-actions">
						<a-tooltip placement="top">
							...
							<a-icon type="search" @click="handleClick(item.liveId, item.type)" />
						</a-tooltip>
						<a-tooltip placement="top">
							<template slot="title">
								<span>详情</span>
		...
		<a-result v-show="!haveData" status="404" title="这里什么都没有" sub-title="快去直播区购买一些课程吧">
			<template #extra>
...
<script>
	...
	export default {
		...
		computed: {
		...mapState(['purchasedLiveCourses']),
		total() {
		return this.purchasedLiveCourses.length;
		},
		haveData() {
		return this.purchasedLiveCourses.length > 0;
		}
	},
		methods: {
			// 跳转到购买页面
			...
			getData(page) {
				axios
					.get(this.$global_msg.userRoute + '/liveCourse/selectMyLive', {
						params: {
