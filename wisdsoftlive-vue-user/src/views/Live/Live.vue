...
<script>
  ...
  import { mockLiveCourses } from '../../mockData.js'; // 引入模拟数据
	export default {
		name: 'Live',
		components: {
			Top,
		...
		methods:{
			...
			selectByRequire(cName, type, page, limit) {
				axios.get(this.$global_msg.liveRoute + '/liveCourse/selectByRequire', {
						params: {
							"cName": cName,
							"type": type,
							"page": page,
      ...
      const filteredCourses = mockLiveCourses.filter((course) => {
      ...
      this.liveCourses = sortedCourses.slice(start, end);
      this.liveTotal = sortedCourses.length;
      this.cName = cName;
      this.type = type;
      this.page = page;
			},
